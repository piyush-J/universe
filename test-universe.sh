...
# Running Universe on working benchmarks
CORPUSFILE="working-benchmarks.yml"
if [ -n "$1" ] && [ $1 = "travis" ]; then
    python3 run-universe-on-corpus.py --corpus-file $CORPUSFILE --is-travis-build true
else
    python3 run-universe-on-corpus.py --corpus-file $CORPUSFILE
fi
